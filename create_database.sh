...
read -p "Enter office system-id [$label] : " input
label=${input:-$label}
...
for sqlfile in schema/*.sql config/*.sql extension/*.sql
do
   ...
   $psql --host=$host --port=$port --dbname=$dbname --username=$username --command="select system.set_system_id('%label%')" > /dev/null 2>> $BUILD_LOG

# Extract the test data from the 7z archive and load it into the database.
echo 
echo "Extracting data files..."
echo "### Extracting data files..." >> $BUILD_LOG 2>&1
# $zip_exe e -y -o"$data_path" "$data_path/sr.7z" >> $BUILD_LOG 2>&1
# Use -p option if the archive is password protected as follows
# $zip_exe e -y -p$archive_password -o"$data_path" "$data_path/sr.7z" >> $BUILD_LOG 2>&1
