...
SET /p label= Enter office system-id [%label%] :
...
%psql% --host=%host% --port=%port% --username=%username% --dbname=%dbname% --command="select system.set_system_id('%label%')" >> %BUILD_LOG% 2>&1
...
REM %zip_exe% e -y -o"%data_path%" "%data_path%sr.7z" >> %BUILD_LOG% 2>&1
REM Use -p option if the archive is password protected as follows
REM %zip_exe% e -y -p%archive_password% -o%data_path% %data_path%sr.7z >> %BUILD_LOG% 2>&1
